...
**The stylesheets are theme-agnostic by scoping.** Every rule in `styles/ranking.css` and `styles/pagination.css` is prefixed with `.rankingTabs` — the placeholder div survives `innerHTML =` and is always the widget's outermost element. The prefix does two jobs: it stops the plugin from restyling a theme's own `.nav-tabs`/`.tab-content`/`.article-item`, and it outranks the theme's single-class rules on the widget's own markup. Keep new selectors prefixed. Spacing follows one rule: **px for structure** (the block margin, gutters, control sizes) because `rem` means 14px in the default theme and something else everywhere else, **em for typography** (gaps between lines of article metadata) so it tracks whatever font size the theme hands down. The widget's own outer spacing is `--ranking-block-spacing` on `.rankingTabs`, which keeps it off its neighbours in every `displayPosition`; the other knobs (`--ranking-gutter`, `--ranking-cover-width`, `--ranking-details-min-width`, and the colour tokens) are declared in the same block and a journal can override any of them from Additional Content. The article row is fluid rather than breakpoint-driven — `.article-item` wraps and `.article-details` has a `flex-basis` of `--ranking-details-min-width` with `min-width: 0` — so the cover drops above the text based on the container's width, not the viewport's, which is what makes it survive a narrow theme column. The one `@media` block (max-width 767px) only tightens spacing and lets the tabs share a row evenly.
